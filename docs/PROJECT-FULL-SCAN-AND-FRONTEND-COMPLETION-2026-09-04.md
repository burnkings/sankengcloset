...
## 10. 2026-09-05 业务评审决策落地

本轮评审以“先让用户完成任务、再扩展运营能力”为原则，已同步落实以下 IA 与交互决策：

| 评审项 | 本轮结论与实现 |
|---|---|
| 趋势话题 | 暂不保留入口。当前没有自定义话题创建与筛选闭环，继续展示固定趋势会造成概念重复；动态发布保留固定系统主题，待未来具备话题实体、创建、审核和筛选能力后再恢复。 |
| 发售日历 | 日期面板改为 BottomSheet；点击商品行直达商品详情，右侧“设置提醒”单独进入提醒编辑。提醒携带 `productId` 与发售批次，预约/抽选默认“蹲新品”，普通开售默认“上新发售”。 |
| 动态关联 | 发布动态同时支持平台商品与个人衣物，多选衣物仅保存衣橱 ID；后端暂无数据时由本地体验数据保证链路可用。 |
| 品牌目录 | 发现页展示去重后的前 4 个精选品牌，使用稳定顺序而非随机，避免刷新造成认知跳动；完整品牌仍在品牌目录页查看。 |
| 榜单 | 新增收藏榜；榜单筛选固定为“榜单类型 → 统计周期 → 坑向”，支持近 7 天/近 30 天。发现页榜单卡删除无信息增量的小字。 |
| 全局文案 | 圈子统一改称“动态”，拉黑/屏蔽统一采用“屏蔽”；反馈浮窗移除左侧竖线；正式展示去除 Mock、演示、预览等内部提示。 |
| 收藏 | “穿搭”改为“动态”，删除合集分支；待购状态调整为“待购状态：已收藏/预售…”的标签顺序；收藏品牌直接进入品牌详情，不再经过搜索页。 |
| 品牌详情 | 品牌目录与收藏品牌统一进入品牌详情；品牌页默认品牌自身系列/商品，不再要求用户二次选择全部坑向；状态采用现货、预售、即将发售、已售罄等行业通用表述。 |
| 价格筛选 | 统一为全部、300 以下、300–800、800–1500、1500 以上五档，控制颗粒度并覆盖主要决策区间。 |
| 衣橱筛选 | 分类计数独立成一行按钮（全部、JK 制服、Lolita、汉服），移除“其他”；排序与“仅看未穿”保留在操作行，避免筛选控件分散到屏幕两端。 |
| 我的动态 | 从简化文本行升级为完整动态卡，补充图片、作者、时间、话题、坑向、关联衣物和删除入口。 |
| 搜索 | 移除全部坑向筛选；保留状态和价格筛选；底部横条改为仅在加载更多时显示轻量加载动画，不再显示含义不明的进度文字。 |

### 本轮后端配合边界

本轮未新增后端接口：日历商品 ID、提醒 `productId/releaseId`、动态 `productId/wardrobeIds` 均沿用现有公开契约；远端无数据时前端使用统一兜底目录。后续真实数据替换只需替换 Service/Store 数据源，不改页面交互协议。
